function varargout = BuckBoostTopology(varargin)
% buckboostTOPOLOGY MATLAB code for buckboostTopology.fig
%      buckboostTOPOLOGY, by itself, creates a new buckboostTOPOLOGY or raises the existing
%      singleton*.
%
%      H = buckboostTOPOLOGY returns the handle to a new buckboostTOPOLOGY or the handle to
%      the existing singleton*.
%
%      buckboostTOPOLOGY('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in buckboostTOPOLOGY.M with the given input arguments.
%
%      buckboostTOPOLOGY('Property','Value',...) creates a new buckboostTOPOLOGY or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before buckboostTopology_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to buckboostTopology_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help buckboostTopology
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @BuckBoostTopology_OpeningFcn, ...
                   'gui_OutputFcn',  @BuckBoostTopology_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before buckboostTopology is made visible.
function BuckBoostTopology_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to buckboostTopology (see VARARGIN)

% Choose default command line output for buckboostTopology
...
% UIWAIT makes buckboostTopology wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = BuckBoostTopology_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in Solutionbuckboost.
function SolutionBuckBoost_Callback(hObject, eventdata, handles)
% hObject    handle to Solutionbuckboost (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%Valor D
if (eq(d_data,0))
    d_sol = (vo_data + vf_data)/(vi_data-vsw_data+vo_data+vf_data);
...
%Valor Vi
if (eq(vi_data,0))
    vi_sol=(vo_sol+vf_data-vo_sol*d_sol+vsw_data*d_sol)/d_sol;
else
    vi_sol=vi_data;
end
...
if (eq(vo_data,0))
    vo_sol= (vi_sol-vsw_data)*(d_sol/(1-d_sol))- vf_data;
else
    vo_sol=vo_data;
end
...
%Valor efficiency
eff_sol=(vo_sol*Io_sol/(IL_sol*d_sol*vi_sol));
