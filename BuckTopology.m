function varargout = BuckTopology(varargin)
% BUCKTOPOLOGY MATLAB code for BuckTopology.fig
%      BUCKTOPOLOGY, by itself, creates a new BUCKTOPOLOGY or raises the existing
%      singleton*.
%
%      H = BUCKTOPOLOGY returns the handle to a new BUCKTOPOLOGY or the handle to
%      the existing singleton*.
%
%      BUCKTOPOLOGY('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BUCKTOPOLOGY.M with the given input arguments.
%
%      BUCKTOPOLOGY('Property','Value',...) creates a new BUCKTOPOLOGY or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before BuckTopology_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to BuckTopology_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help BuckTopology

% Last Modified by GUIDE v2.5 29-Nov-2018 19:41:13

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @BuckTopology_OpeningFcn, ...
                   'gui_OutputFcn',  @BuckTopology_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before BuckTopology is made visible.
function BuckTopology_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to BuckTopology (see VARARGIN)

% Choose default command line output for BuckTopology
...
% UIWAIT makes BuckTopology wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = BuckTopology_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in SolutionBuck.
function SolutionBuck_Callback(hObject, eventdata, handles)
% hObject    handle to SolutionBuck (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%Valor D
if (eq(d_data,0))
    d_sol = (vo_data + vf_data)/(vi_data - vsw_data + vf_data);
...
%Valor Vi
if (eq(vi_data,0))
    vi_sol=(((vo_sol+vf_data)/d_sol)- vf_data - vsw_data);
else
    vi_sol=vi_data;
end
...
%Valor Vo
if (eq(vo_data,0))
    vo_sol=(d_data*(vf_data-vsw_data+vi_sol) - vf_data);
else
    vo_sol=vo_data;
end
...
%Valor Io and IL
    ...
    elseif (CCM == 0 && (~eq(L_data,0)) && (eq(AiLpos,0)))
         AiLpos_sol = ((vi_sol-vsw_data-vo_sol)*d_sol*(1/FS_data))/L_data;
    end
...
%         
  if ((eq(AiLpos,0)) && (~eq(L_data,0)))
    AiLpos_sol = ((vi_data-vsw_data-vo_sol)*d_sol*(1/FS_data))/L_data;
    IL_sol=(AiLpos_sol/2);
    Io_sol=IL_sol;
...
if (eq(L_data,0))
   L_sol=(((vi_data-vo_data-vsw_data)*d_sol*(1/FS_data))/AiLpos_sol)*10^6;
else
    L_sol=L_data*10^6;
end

%Valor ID and Isw
Id_sol= Io_sol*(1-d_sol);
Isw_sol=Io_sol*(d_sol);

%Valor C and AVO
C_data=C_data*10^(-6);
if (eq(AVO_data,0) && ~eq(C_data,0))
    AVO_sol= ((AiLpos_sol*(1/FS_data))/(8*C_data));
    C_sol=C_data*(10^6);
elseif (eq(C_data,0) && ~eq(AVO_data,0))
    C_sol =((AiLpos_sol*(1/FS_data))/(8*AVO_data))*(10^6);
    AVO_sol= AVO_data;
else
    C_sol='NaN';
    AVO_sol='NaN';
